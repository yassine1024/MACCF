...
public class MAJDB {
    ...
    private void LoadFileCompteBloquer(String Path) throws SQLException {
        ...
        req = "INSERT INTO COMPTE_BLOQUER(NUM_CPT,DT_BLOC,DT_DEBLOC,MOTIF_BLOC,MT_OBJET_BLOC)" +
                "SELECT NUM_CPT,DATE_FORMAT(STR_TO_DATE(DT_BLOC, '%d-%b-%y'), '%Y-%m-%d')," +
                "DATE_FORMAT(STR_TO_DATE(DT_DEBLOC, '%d-%b-%y'), '%Y-%m-%d')," +
                "MOTIF_BLOC,MT_OBJET_BLOC" +
                " FROM temporary_COMPTE_BLOQUER " +
                "	ON DUPLICATE KEY UPDATE " +
                "	NUM_CPT = VALUES(NUM_CPT)," +
                "	DT_BLOC = VALUES(DT_BLOC)," +
                "	DT_DEBLOC = DATE_FORMAT(STR_TO_DATE(temporary_COMPTE_BLOQUER.DT_DEBLOC, '%d-%b-%y'), '%Y-%m-%d')," +
